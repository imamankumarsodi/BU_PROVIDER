//
//  ScheduelServicesVC+CustomMethods.swift
//  BU_PROVIDER
//
...
import Foundation
extension ScheduelServicesVC{
    //TODO: Navigation setup implenemtation
    internal func navSetup(){
        self.tabBarController?.tabBar.isHidden = true
        super.setupNavigationBarTitle(AppColor.header_color,false,ConstantTexts.ScheduledServicesLT, AppColor.whiteColor, leftBarButtonsType: [.back], rightBarButtonsType: [.empty])
...
extension ScheduelServicesVC{
    internal func hitGetScheduelServices(){
        ...
        ServiceClass.shared.webServiceBasicMethod(url: SAuthApi.providerSchedulList, method: .post, parameters: parameters, header: nil, success: { (result) in
            ...
            if let result_Dict = result as? [String:Any]{
                ...
                if let code = result_Dict["status"] as? Int{
                    if code == 200{
                        self.resDataSchedueldVM = ResDataSchedueld(fromDictionary: result_Dict)
