//
//  PersonalDetailsVC+CustomMethods.swift
//  BU_PROVIDER
//
...
extension PersonalDetailsVC{
    ...
    internal func navSetup(){
        ...
        super.setupNavigationBarTitle(AppColor.header_color,false,ConstantTexts.personal_Details_LT, AppColor.whiteColor, leftBarButtonsType: [.back], rightBarButtonsType: [])
    ...
    internal func initValues(){
        ...
        if isComingForEditing{
            ...
            self.dataListVM?.dataStoreStructs[0].value = user.parent_name
            self.dataListVM?.dataStoreStructs[1].value = user.gender
            self.dataListVM?.dataStoreStructs[2].value = user.dob
        }
...
extension PersonalDetailsVC{
    ...
    internal func hitUpDateProfile(){
        ...
        guard let showDate = inputFormatter.date(from: dataListVM_T.dataStoreStructAtIndex(2).value) else {
            print("No date found...")
            return
        }
        ...
        let genderString = dataListVM_T.dataStoreStructAtIndex(1).value == "Male" ? "M" : "F"
        ...
        let parameters = [Api_keys_model.provider_id:user.id,
                          ...
                          Api_keys_model.parent_name:dataListVM_T.dataStoreStructAtIndex(0).value,
                          Api_keys_model.gender:genderString,
                          Api_keys_model.dob:resultString] as [String:AnyObject]
            ...
            if let result_Dict = result as? NSDictionary{
                if let code = result_Dict.value(forKey: "status") as? Int{
                    ...
                    if code == 200{
                        ...
                        user_saved.parent_name = dataListVM_T.dataStoreStructAtIndex(0).value
                        user_saved.gender = dataListVM_T.dataStoreStructAtIndex(1).value
                        user_saved.dob = dataListVM_T.dataStoreStructAtIndex(2).value
